...
{
    local temp="$KUBEBASE_SESSION_DIR/.kubeconfig.scope.tmp.$$"
...
command_emit_namespace()
{
    ...
    if [ "$selector" = "--clear" ]; then
        ...
        update_session_scope \
            "$( [ -n "${KUBEBASE_PROJECT:-}" ] && printf project || printf cluster )" \
            "${KUBEBASE_PROJECT:-}" \
            "" \
            ""
...
command_emit_project()
{
    ...
    clear_effective_namespace
    update_session_scope "project" "$selector" "" ""
